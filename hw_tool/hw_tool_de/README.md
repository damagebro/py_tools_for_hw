...
| tool            | source_path                                          | 说明                                  |
| --------------- | ---------------------------------------------------- | ------------------------------------- |
| `mem_tool`      | `py_tools_for_hw:mem_tool/src/main.py`               | 生成 memory shell、报告和集成 RTL     |
...
DE 工具都要求直接支持 `--help`，并统一使用返回码 `0`（成功）、`1`（业务失败）、`2`（参数错误）。`--version` 为可选能力。注册表登记 README、Git URL、入口脚本、最小样例、smoke 参数、预期输出与单元测试入口。

| tool            | 最小样例                            | smoke 关键输出                  |
| --------------- | ----------------------------------- | ------------------------------- |
| `mem_tool`      | `mem_tool/tests/fixtures/spram.lst` | `smoke_spram_shell.sv`          |
...
`verify` 只检查契约和注册信息；`test --unit` 运行工具单元测试；`test --smoke` 在受控临时目录生成最小输出；`test --all` 执行两者。

## 工具来源仓库

当前所有 DE 工具共用 `py_tools_for_hw` Git URL 与一个 checkout。注册表只记录仓库内相对路径，`sync --all` 会按来源仓库去重，同一仓库只同步一次。
...
它会 clone 到 `hw_tool/repository/py_tools_for_hw/`。后续可使用 `hw_tool de sync <tool>` 或 `hw_tool sync --all` 更新。
