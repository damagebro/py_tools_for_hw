...
`data_mode=txt` 时，`apb_master` 忽略 `base_addr`、`byte_size` 和 `rw_mode`，按 `data_file` 中的 `addr data` 对顺序逐笔发起 APB 写传输；空白、空行和 `#` 之后的注释均会忽略。`data_file` 路径相对仿真运行目录 `bin/`，可直接使用 `apb_master.txt`：
