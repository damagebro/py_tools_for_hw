...
task automatic load_cfg();
    ...
    pstrb_cfg = cfg_get_uint(cfg_file, {cfg_prefix, "pstrb"}, {APB_SW{1'b1}});
endtask
...
task automatic load_file_data();
    ...
    if (fd == 0) begin
        $display("[APB_M%0d] warning: cannot open data_file=%s", MASTER_ID, data_file);
        return;
    end
    ...
    $fclose(fd);
    $display("[APB_M%0d] loaded %0d bytes from %s", MASTER_ID, file_size, data_file);
endtask
...
task automatic append_txt_token(
    ...
    if (expect_addr) begin
        ...
        item.data = APB_DW'(str_to_uint(token));
...
task automatic load_txt_data();
    ...
    txt_items.delete();
    if (data_file == "") begin
        $display("[APB_M%0d] warning: txt mode needs data_file", MASTER_ID);
        return;
    end
    ...
    if (!expect_addr)
        $display("[APB_M%0d] warning: ignored dangling addr token %s", MASTER_ID, addr_token);
    $display("[APB_M%0d] loaded %0d text transactions from %s", MASTER_ID, txt_items.size(), data_file);
endtask

function automatic logic [APB_DW-1:0] build_data(
    ...
    data = '0;
    for (int idx = 0; idx < APB_SW; idx++) begin
...
task automatic apb_transfer(
    ...
    m_apb.pwdata  <= write_data;
    m_apb.pstrb   <= APB_SW'(pstrb_cfg);
    m_apb.psel    <= 1'b1;
    m_apb.penable <= 1'b0;
    @(posedge clk);
    m_apb.penable <= 1'b1;
...
initial begin
    ...
    if (!enable) begin
        ...
        $display("[APB_M%0d] disabled", MASTER_ID);
    end else begin
        load_file_data();
        if ((data_mode == "txt") || (data_mode == "text")) begin
